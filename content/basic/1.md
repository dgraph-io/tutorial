...
Dgraph identifies each node with a unique internal id, it's UID --- it's not an edge, but can be returned in a query with `_uid_`.  If you already know a node's UID, it can be retrieved with `func: "uid(<uid-number>)"`.
